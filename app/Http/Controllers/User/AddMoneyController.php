...
{
    use Stripe,Manual,FlutterwaveTrait,RazorTrait,SslcommerzTrait,Fiat24Trait,TransactionAgent;
    ...
    /**
     * Fiat24 Payment Success Callback
     */
    public function fiat24PaymentSuccess($trx)
    {
        $token = $trx;
        $checkTempData = TemporaryData::where("type", PaymentGatewayConst::FIAT24)
            ->where("identifier", $token)
            ->first();
        
        if (!$checkTempData) {
            return redirect()->route('user.add.money.index')
                ->with(['error' => [__("Transaction Failed. The record didn't save properly. Please try again")]]);
        }
        ...
        try {
            PaymentGatewayHelper::init($checkTempData)
                ...
                ->responseReceive('fiat24');
        } catch (Exception $e) {
            return back()->with(['error' => [__("Something went wrong! Please try again.")]]);
        }
        
        return redirect()->route("user.add.money.index")
            ->with(['success' => [__('Successfully Added Money via Fiat24')]]);
    }

    /**
     * Fiat24 Payment Cancel Callback
     */
    public function fiat24PaymentCancel($trx)
    {
        $token = $trx;
        $checkTempData = TemporaryData::where("type", PaymentGatewayConst::FIAT24)
            ->where("identifier", $token)
        ...
        if ($checkTempData) {
            $checkTempData->delete();
        }
        
        return redirect()->route('user.add.money.index')
            ->with(['error' => [__('Fiat24 payment cancelled')]]);
    }
