...
<section class="banner-section bg_img gradient-scroll" data-background="{{ asset('public/frontend/') }}/images/banner/bg-1.jpg">
    <div class="container home-container">
        <div class="row mb-30-none align-items-center">
            <div class="col-lg-6 col-md-6 mb-30">
                <div class="banner-thumb-area text-center parallax-element" data-speed="0.05">
                    <img src="{{ get_image(@$banner->value->images->banner_image,'site-section') }}" alt="banner" class="float-animation">
                </div>
            </div>
            <div class="col-lg-6 col-md-6 mb-30">
                <div class="banner-content scroll-reveal">
                    ...
                    <div class="app-btn-area">
                        <a href="{{ @$app_urls->android_url }}" class="app-btn btn-premium magnetic-btn ripple-effect" target="_blank">
                            <div class="icon">
                        ...
                        <a href="{{ @$app_urls->iso_url }}" class="app-btn btn-premium magnetic-btn ripple-effect" target="_blank">
                            <div class="icon">
...
<section class="banner-floting-section">
    <div class="container">
        <div class="row">
            <div class="col-xl-12">
                <div class="banner-floting-right-area glass-card scroll-reveal">
                    <ul class="banner-floting-right-list stagger-list">
                        @if(isset($banner_floting->value->items))
                            @foreach($banner_floting->value->items ?? [] as $key => $item)
                            <li class="stagger-item"><i class="fas fa-check"></i>{{ $item->language->$lang->name ?? $item->language->$system_default->name }}</li>
                            @endforeach
                        @endif
                    </ul>
                    <div class="banner-floting-right-content">
                        ...
                        <a href="{{url('/').'/'.@$banner_floting->value->language->$lang->button_link}}" class="link-area animated-link">{{ __($banner_floting->value->language->$lang->button_name ?? $banner_floting->value->language->$system_default->button_name) }} <i class="fas fa-long-arrow-alt-right ms-1"></i></a>
                    </div>
                </div>
            </div>
        </div>
